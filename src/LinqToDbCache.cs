...
{
    /// <summary>
    /// IDistributedCache using Linq2Db.
    /// </summary>
    public class LinqToDbCache : IDistributedCache
    {
        public const int MAX_KEYLENGTH = 449;
        ...
        public LinqToDbCache(IOptions<LinqToDbCacheOptions> options)
        {
            ...
            if (string.IsNullOrEmpty(cacheOptions.ConnectionString))
            {
                throw new ArgumentException(
                    $"{nameof(LinqToDbCacheOptions.ConnectionString)} cannot be empty or null.");
            }
            if (string.IsNullOrEmpty(cacheOptions.SchemaName))
            {
                throw new ArgumentException(
                    $"{nameof(LinqToDbCacheOptions.SchemaName)} cannot be empty or null.");
            }
            if (string.IsNullOrEmpty(cacheOptions.TableName))
            {
                throw new ArgumentException(
                    $"{nameof(LinqToDbCacheOptions.TableName)} cannot be empty or null.");
            }

            if (string.IsNullOrEmpty(cacheOptions.ProviderName))
            {
                throw new ArgumentException(
                    $"{nameof(LinqToDbCacheOptions.ProviderName)} cannot be empty or null.");
            }
            ...
            {
                throw new ArgumentException(
                    $"{nameof(LinqToDbCacheOptions.ExpiredItemsDeletionInterval)} cannot be less than the minimum " +
